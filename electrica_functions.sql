...
CREATE OR REPLACE FUNCTION calcular_costo_actual(alquiler_id INT)
...
DECLARE
    ...
    costo DECIMAL(10,2);
BEGIN
    SELECT t.valor_por_minuto INTO tarifa_por_minuto
    FROM alquileres a
...
CREATE OR REPLACE FUNCTION esta_disponible(patineta_id INT)
RETURNS BOOLEAN AS $$
DECLARE
    estado INT;
BEGIN
    SELECT estado_id INTO estado FROM patinetas WHERE patineta_id = patineta_id;
    RETURN estado = 1;
END;
$$ LANGUAGE plpgsql;